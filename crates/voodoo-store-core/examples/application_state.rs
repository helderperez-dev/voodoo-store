...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (job_id, event_id) = {
        ...
        let job_id = tx.enqueue_job(JobSpec::new(b"email.send_receipt", b"order:42"), 1_000)?;
    ...
    let job = store.get_job(&job_id)?.expect("committed job");
    println!("job_handler={}", String::from_utf8_lossy(&job.handler));
